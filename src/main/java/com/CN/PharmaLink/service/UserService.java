...
String jwtToken  = token.replace("Bearer ","");
		List<MedicalStoreDto> nearestMedicalStoreList = storeFinderCommunicator.getNearestMedicalStores(userId, distance,jwtToken );
return nearestMedicalStoreList;
	}

	public List<MedicalStoreDto> getMedicalStoresWithMedicine(String medicine , String token){
		String jwtToken  = token.replace("Bearer ","");
		List<MedicalStoreDto> medicalStoreListHavingMedicine = storeFinderCommunicator.getMedicalStoresWithMedicine(medicine,jwtToken);
		return medicalStoreListHavingMedicine;
	}
}
